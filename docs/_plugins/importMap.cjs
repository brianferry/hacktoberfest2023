    ...
    try {
      ...
      for (const x of await glob('./*/*.ts', { cwd: elementsDir, dotRelative: true, ignore: '**/*.d.ts' })) {
        traces.push(
          ...
          generator.traceInstall(x.replace('./', '@hack/elements/').replace('.ts', '.js')),
        );
      }
      await Promise.all(traces);

      generator.importMap.replace(pathToFileURL(elementsDir).href, '/assets/packages/@hack/elements/elements/');
      generator.importMap.replace(pathToFileURL(elementsDir).href.replace('elements', 'lib'), '/assets/packages/@hack/elements/lib/');
      ...
      // HACK: this was interfering with the @patternfly/pfe-core default imports
      delete json.scopes?.['/assets/packages/']?.['@patternfly/'];
      // ENDHACK
      
      // TODO: automate this
      Object.assign(json.imports ?? {}, {
        '@rhds/elements': 'https://ga.jspm.io/npm:@rhds/elements@1.0.1/rhds.min.js',
      });
